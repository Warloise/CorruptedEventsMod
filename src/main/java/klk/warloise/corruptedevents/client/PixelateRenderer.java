...
public class PixelateRenderer {
    ...
    private static final ResourceLocation PIXELATE_SHADER = new ResourceLocation(Corruptedevents.MODID, "shaders/post/pixelate.json");

    private static PostChain pixelateShader;
    ...
    private static void makePixelateShader() {
        if (pixelateShader == null) {
            pixelateShader = createShaderGroup(PIXELATE_SHADER);
        }
    }
    ...
    public static void onRender(TickEvent.RenderTickEvent event) {
        ...
        if (player != null) {
            ...
            if (player.hasEffect(ModEffects.PIXELATE.get())) {
                activeShader = pixelateShader;
            }
