...
public class CRTRenderer {
    ...
    private static final ResourceLocation CRT_SHADER = new ResourceLocation(Corruptedevents.MODID, "shaders/post/crt.json");

    private static PostChain crtShader;
    ...
    private static void makeCRTShader() {
        if (crtShader == null) {
            crtShader = createShaderGroup(CRT_SHADER);
        }
    }
    ...
    public static void onRender(TickEvent.RenderTickEvent event) {
        ...
        if (player != null) {
            ...
            if (player.hasEffect(ModEffects.CRT.get())) {
